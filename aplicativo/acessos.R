...
# ImportaÃ§Ã£o dos dados da Lista do Excel com todos os documentos da IFI
itens <- read_excel(path = "Lista.xlsx")



# DefiniÃ§Ã£o da funÃ§Ã£o para extrair dados do site da IFI
...
count.ifi <- function(){
  ...
  for (i in 1:nrow(RAFs)){    store[[i]] <- count.views.now(RAFs[i,"CÃ³digo"])    }
...
# Abrir a base01, que contÃ©m os dados da IFI brutos. 
#load(file = "base01")
...
atualizador <- function(){
  ...
  for(i in 1:NROW(itens)){
    ...
    if(#i == 1 & 
      ...
      names(teste01.total) <- c("Nome_Arquivo","visualizaÃ§Ãµes","ID","Nome","Categoria","Numero","Data", "Data_Original")
    ...
    else
    {
      ...
      names(tmp.total) <- c("Nome_Arquivo","visualizaÃ§Ãµes","ID","Nome","Categoria","Numero","Data", "Data_Original")
  ...
  teste02.total <- teste02.total %>% 
    ...
    summarise(`visualizaÃ§Ãµes`=sum(`visualizaÃ§Ãµes`))
  ...
  # Seleciona o Ãºltimo valor de cada data
